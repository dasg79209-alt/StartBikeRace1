...
# Pygame এবং Mixer ইনিশিয়ালাইজেশন
pygame.init()
pygame.mixer.init()
...
# রঙসমূহ (GRAY এখানে উপরে ডিফাইন করা হয়েছে)
...
try:
    ...
    pygame.mixer.music.play(-1) # মিউজিক অটো প্লে হবে
except: 
    bike_img = car_img = None
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if width//2-100 <= mx <= width//2+100:
                    if 400 <= my <= 450: return
                    if 480 <= my <= 530: shop_menu()
...
def game_loop():
    ...
    current_bike_img = bike_img.copy() if bike_img else None
    if bike_type == "red" and current_bike_img:
    ...
    while True:
        ...
        for c in coins:
            c[1] += speed
            if c[1] > height: 
                c[1], c[0] = random.randint(-500, -100), random.randint(110, 445)
        ...
        for e in enemies:
            e[1] += speed
            if e[1] > height: 
                e[1], e[0] = random.randint(-600, -100), random.randint(110, 445)
                score += 1
        ...
        if current_bike_img: screen.blit(current_bike_img, (bike_x, bike_y))
        
        # স্কোরবোর্ড দেখানো
        score_text = font_small.render(f"Score: {score}  Coins: {current_coins}", True, WHITE)
